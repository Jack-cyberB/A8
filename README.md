...
- `scripts/init_mysql_storage.py` 会创建 `a8` 数据库、建表并把当前展示集/分析/运行态数据导入 MySQL。
- `STORAGE_BACKEND` 默认值已改为 `mysql`；文件数据仅保留为初始化导入源和测试夹具。
- 前端 API 路径保持不变。
...
运行态默认写入 MySQL 的 `ai_calls / anomaly_actions / anomaly_notes / system_snapshots` 表；`data/runtime/*` 仅保留为初始化导入源与测试夹具。
`scripts/run_regression.py` 会同步生成 `data/runtime/regression_summary.json`，并在 MySQL 模式下尽量同步写入 `system_snapshots`。
